document.addEventListener('DOMContentLoaded', () => {
  ...
  const navLinks = document.querySelectorAll('header nav a');document.addEventListener('DOMContentLoaded', () => {
  const menuIcon = document.querySelector('#menu-icon');
  const navbar = document.querySelector('.navbar');
  const homeSci = document.querySelector('.home-sci');
  const sections = document.querySelectorAll('section');
  ...
  window.addEventListener('scroll', () => {
    ...
    // Add transparency to footer background when scrolling down
    if (top > lastScrollTop){
      footer.classList.add('transparent-bg');
    } else {
      footer.classList.remove('transparent-bg');
    }
